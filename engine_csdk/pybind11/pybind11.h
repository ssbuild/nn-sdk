...
public:
    ...
    static PyObject* import_ex(const char* name) {
        PyObject* obj = NULL;
		auto o_name = PyUnicode_FromString(name);
#if (PY_MAJOR_VERSION == 3 && PY_MINOR_VERSION >=7 )
        obj = PyImport_GetModule(o_name);
#endif
        if (!obj) {
            obj = PyImport_ImportModuleLevelObject(o_name, NULL, NULL, NULL, 0);
        }
		 Py_DECREF(o_name);
        return obj;
    }
